...
export class EditProductPhotosComponent implements OnInit {
  ...
  uploadFile(file: File) {
    this.adminService.uploadImage(file, this.product.id).subscribe({next:(event: HttpEvent<any>) => {
      switch (event.type) {
  ...
  deletePhoto(photoId: number) {
    this.adminService.deleteProductPhoto(photoId, this.product.id).subscribe({next:() => {
